"""JWT bearer-token authentication for the pdf2nhcx (NHCX Insurance) API.

Two accepted token types
------------------------
1. **Demo tokens** — HS256 JWT signed by this service's ``NHCX_SECRET_KEY``.
...
Auth bypass
-----------
Set ``NHCX_AUTH_ENABLED=false`` to disable enforcement entirely —
...
logger = logging.getLogger("pdf2nhcx.auth")
...
def _auth_enabled() -> bool:
    return os.getenv("NHCX_AUTH_ENABLED", "true").lower() not in ("false", "0", "no")


def _secret_key() -> str:
    return os.getenv("NHCX_SECRET_KEY", "")
...
def _validate_demo_token(token: str) -> Dict[str, Any]:
    ...
    if not secret:
        raise HTTPException(
            ...
            detail="Demo token validation is unavailable: NHCX_SECRET_KEY is not configured.",
        )
    try:
        claims = jwt.decode(
            token,
...
async def require_bearer(request: Request) -> Dict[str, Any]:
    ...
    Bypasses auth entirely when ``NHCX_AUTH_ENABLED=false``.
    """
    if not _auth_enabled():
        logger.debug("Auth bypassed (NHCX_AUTH_ENABLED=false)")
...
def issue_demo_token(name: str, email: str, expiry_days: int = 1) -> str:
    ...
    if not secret:
        raise HTTPException(
            status_code=503,
            detail="Demo tokens are not available: NHCX_SECRET_KEY is not configured.",
        )
    now = int(time.time())
    payload = {
        "sub": email,
