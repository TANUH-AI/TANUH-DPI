"""
docling_engine.py — Docling OCR adapter (primary engine)

Best for: native digital PDFs with embedded text, rich structure,
          tables, headings, multi-column layouts.

Docling (IBM Research) parses PDFs structurally — it understands the
document model rather than treating the page as a flat image.
Output quality on digital PDFs is significantly better than any
pixel-based OCR approach.
...
def run(
    ...
    Returns:
        Markdown string on success, None if Docling is unavailable or output is empty.
    """
    try:
        ...
        logger.warning(
            "docling unavailable — skipping Docling engine (%s). "
            "Install with: pip install 'docling>=2.0,<3'",
            _import_err
        )
        return None
    ...
    try:
        ...
        pipeline_options.do_ocr = True          # enable OCR for scanned pages inside otherwise digital PDFs
        ...
        logger.info("Docling: extracted %d/%d pages from %s",
